...
def start_fixed_batch(sm: sem.Simulator):
    ...
    while True:
        ...
        retrieved_feedbacks = forward_real_packets_or_retrieve_acks_for_fixed_batch(sm, current_epoch_selected_path)
        ...
        # 2.5 如果不是 deda 算法 (只是带有延迟的, fixed batch, osmd 算法)
        if not sm.simulator_params.enable_deda_algorithm:
            # 2.5.1 由于可能获取到不止一个 feedback, 所以需要进行每个 feedback 的遍历
            for retrieved_feedback in retrieved_feedbacks:
        ...
        # 2.6 如果是 deda 算法 (delay-adpative algorithm)
        else:
